...
if __name__ == "__main__":
    ...
    for i_round, (max_d, thr_val, avg_threshold, avg_ratio, alpha) in enumerate(list(itertools.product(max_ds, thr_vals, avg_thresholds, avg_ratios, alphas))):
        ...
        uu.save_gpickle(r'results//fitted_models//proposed_revision//seoul//seoul_weekends_model8_%s_%s_%s_%s_%s_%s.pickle'%(max_d, max_d_2, str(thr_val), avg_threshold, avg_ratio, str(alpha)), reg1)
    ...
    #pd.concat([result_df, result_df1]).reset_index(drop=True).to_csv(r'y_val_cdf_weight_seoul_weekends_model8_lasso_avg_threshold_ratio.csv', index=False)
