...
def get_pop_info(idx_dic, census_weight_dic, census_idx_dic, static_df):
    ...
    for (uni_key1, uni_val1), (uni_key2, uni_val2), (uni_key3, uni_val3) in zip(idx_dic.items(), census_weight_dic.items(),
        ...
        newa1= (static_df.loc[static_df['tot_oa_cd'].isin(uni_val3.values())])[['tot_oa_cd','value']]
        newa2 = newa1.groupby('tot_oa_cd').sum()

        temp_dict1 = dict(zip(newa2.index.values, newa2['value']))
...
if __name__ == "__main__":
    ...
    shp_path = r'C:\Users\dohyeon\Desktop\2020_집계구경계(SGIS)/bnd_oa_00_2020_2020_2Q.shp'
    temp_shp = read_shp(shp_path)
    year_num = 2019
    #%%
    aaa2=pd.read_table(r'C:\Users\dohyeon\Desktop\2018-2019년_집계구별_통계(2020기준)\2020년기준_%s년_산업분류별(10차_대분류)_총괄사업체수.txt'%(year_num), sep='^',  ).fillna(0)
    aaa3=pd.read_table(r'C:\Users\dohyeon\Desktop\2018-2019년_집계구별_통계(2020기준)\2020년기준_%s년_산업분류별(10차_대분류)_종사자수.txt'%(year_num), sep='^',  ).fillna(0)
    aaa4 = pd.read_table(r'C:\Users\dohyeon\Desktop\2018-2019년_집계구별_통계(2020기준)\2020년기준_%s년_세대구성별가구.txt'%(year_num), sep='^').fillna(0)
    aaa5 = aaa4.loc[aaa4.item=='ga_sd_001'].fillna(0).reset_index(drop=True)
    aaa6 = pd.read_table(r'C:\Users\dohyeon\Desktop\2018-2019년_집계구별_통계(2020기준)\2020년기준_%s년_성연령별인구.txt'%(year_num), sep='^').fillna(0)

    tot_2049_codes = ['in_age_035', 'in_age_036', 'in_age_037', 'in_age_038'] + ['in_age_065', 'in_age_066', 'in_age_067', 'in_age_068']
    aaa7 = aaa6.loc[aaa6.item.isin(tot_2049_codes)].fillna(0).reset_index(drop=True)

#%%
    MultiPolygon([Polygon(yy[0]) for yy in xx['geometry']['coordinates']])

    #%%
    shp_list = []
    for xx in temp_shp:

        if (len(xx['geometry']['coordinates']) == 1) & (isinstance(xx['geometry']['coordinates'], list)):
            shp_list.append(Polygon(xx['geometry']['coordinates'][0]))
        elif len(xx['geometry']['coordinates']) == 1 & isinstance(xx['geometry']['coordinates'], float):
            shp_list.append(Polygon(xx['geometry']['coordinates']))

        else:
            try:
                shp_list.append(MultiPolygon([Polygon(yy[0]) for yy in xx['geometry']['coordinates']]))
            except:
                shp_list.append(MultiPolygon([Polygon(yy) for yy in xx['geometry']['coordinates']]))
...
#%%
    ...
    tot_pop_dict = get_pop_info1(bin_dict, intersect_dict, new_bin_bin_dict, aaa6)
