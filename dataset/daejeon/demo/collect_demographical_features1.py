...
#%%
    ...
    one_intersect_li = [(xx,yy) for xx,yy in enumerate(shp_list) if yy.intersection(shp_list[2195])]

    MultiPolygon([yy for xx,yy in one_intersect_li])

    temp1z=shp_list[2195].intersection(shp_list[2194])
    temp
    MultiPolygon([shp_list[2194],shp_list[2195] ])

    MultiPolygon([shp_list[2196],shp_list[2195] ])

    temp2z = shp_list[2195].intersection(shp_list[2196])
    ...
    nn1 = [int(temp_shp[xx]['properties']['TOT_REG_CD']) for xx,yy in one_intersect_li]

    [xx for xx,yy in one_intersect_li]
    ...
    avg_target_df.loc[    np.intersect1d(avg_target_df.index, nn1)].sum()
    avg_target_df1.loc[    np.intersect1d(avg_target_df1.index, nn1)].sum()


    np.intersect1d(avg_target_df1.index, nn1)


    avg_target_df.loc[nn1]

    np.intersect1d(avg_target_df.index, nn1)

    #%%
    import matplotlib.pyplot as plt

    #%%
    plt.plot(avg_target_df.values)
    plt.boxplot(avg_target_df.values, showmeans=True)

    plt.boxplot(avg_target_df.loc[nn1].replace({0:np.nan}).dropna().values, showmeans=True)


    avg_target_df.mean()

    avg_target_df.loc[nn1].sum() / 9


    avg_target_df.loc[    np.intersect1d(avg_target_df.index, nn1)]
    #%%
    ...
    [idx_abs_rel_dict[xx] for xx in zero_ga_non_pop_index]

    #%%

    #one_intersect_li = [(xx,yy) for xx,yy in enumerate(shp_list) if yy.intersection(shp_list[1831]) & if not yy.equals(shp_list[1831])]

    one_intersect_li = []
    for xx,yy in enumerate(shp_list):
        if yy.intersection(shp_list[2947]):
            if not yy.equals(shp_list[2947]):
                one_intersect_li.append((xx,yy))

    #%%

    avg_target_df.loc[[idx_rel_abs_dict[xx] for xx in [zz for zz,yy in one_intersect_li]]]


    avg_target_df1.loc[[idx_rel_abs_dict[xx] for xx in [zz for zz,yy in one_intersect_li]]]



    #%% ga_df 만들어보기

    avg_target_df.loc[zero_ga_non_pop_index]
#%%
    zzzz1 = []
    ...
    #%%
    intersect_dict[1].values()
    np.sum(bbb1.loc[new_bin_bin_dict[1].values(), 'tot_ga'] *  list(intersect_dict[1].values()))
