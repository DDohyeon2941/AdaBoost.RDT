...
if __name__ == "__main__":
    ...
    p1 = pd.read_csv(r'weather/weather_2018.csv')
    p1 = p1.rename(columns={'rain_log2':'log_rain2'})



    p2 = pd.read_csv(r'weather/weather_2019.csv')
    ...
    p1.loc[:, 'date'] = pd.to_datetime(p1.time)
    ...
    p2.loc[:, 'date'] = pd.to_datetime(p2.time)
    ...
    demo_path = r'demo\demo_500m_2018.csv'
    geo_path = r'geo\geo_500m.csv'
    d1, g1 = pd.read_csv(demo_path), pd.read_csv(geo_path)

    date_df = pd.read_csv(r'station_info_2018.csv')
    dg1 = pd.merge(left=d1, right=g1, left_on='ID', right_on='ID')

    new_dg = dg1.loc[dg1.ID.isin(date_df['0'])].reset_index(drop=True)
    new_dg
    ...
    date_df_trn = date_df[['0','1','2']].dropna().astype({'0':int})
    date_df_trn.loc[:,'1'] = pd.to_datetime(date_df_trn['1'])
    date_aa=date_df_trn.loc[date_df_trn['1'] > date_df_trn['1'][0]]
    ...
    del_list = np.array([])
    for uni_stat, uni_date in zip(date_aa['0'],date_aa['1']):
    ...
    #%% 2019
    demo_path = r'demo\demo_500m_2019.csv'
    geo_path = r'geo\geo_500m.csv'
    d1, g1 = pd.read_csv(demo_path).fillna(0), pd.read_csv(geo_path)

    date_df = pd.read_csv(r'station_info_2019.csv')
    dg1 = pd.merge(left=d1, right=g1, left_on='ID', right_on='ID')
    ...
    date_df_tst = date_df[['0','1','2']]

    date_df_tst.loc[:,'2'] = pd.to_datetime(date_df_tst['2'])
    date_aa=date_df_tst.loc[date_df_tst['2'] < date_df_tst['2'][0]]
    date_aa


    del_list = np.array([])
    for uni_stat, uni_date in zip(date_aa['0'],date_aa['2']):
        del_list = np.append(del_list,aa3.loc[(aa3.ID==uni_stat)&(aa3.r_date>uni_date)].index.values)
