...
if __name__ == "__main__":
    ...
    # 0
    temp_df = pd.read_csv(r'results/bootstrapped/proposed/%s/%s/y_val_cdf_weight_%s.csv'%(city_name, day_type, 0))
...
#%%
    ...
    temp_df1 = pd.read_csv(r'results/bootstrapped/proposed/%s/%s/y_val_%s.csv'%(city_name, day_type, 4))

    get_group_metric_df_extreme(temp_df1, get_grouper_index(temp_df1.real.values, 4, 40))
...
#%% seoul, weekends
    ...
    temp_df = pd.read_csv(r'results/bootstrapped/proposed/%s/%s/y_val_cdf_weight_%s.csv'%(city_name, day_type, 4))
    temp_df = (temp_df.loc[temp_df['info'] == 'pred_y'])[temp_df.columns[:-1]].reset_index(drop=True)
