...
if __name__ == "__main__":
    ...
    total_round = len(day_types) * len(city_names) * rs_num

    ii=0
    for city_name, day_type in itertools.product(city_names, day_types):
        ...
        idx_df = pd.read_csv(r'preprocess/bootstrap/bootstrap_index_%s_%s_no_extreme_20240501.csv'%(city_name, day_type))
        n_est, max_d = [50, 4]
        ada_n_est = 50

        for random_state in np.arange(rs_num):
            ...
            result_df.to_csv(r'results/bootstrapped/comparison/%s/%s/y_val_no_extreme_%s.csv'%(city_name, day_type, random_state),index=False)
            print('save %s - %s %s rs %s th results csv file'%(city_name, day_type, random_state, ii))
            print('-'*50)
            ii += 1
    ...
    for city_name, day_type in itertools.product(city_names, day_types):
        trn_df = pd.read_csv(r'preprocess/bootstrap/2018_train_%s_%s_20240418.csv'%(city_name, day_type))
        tst_df = pd.read_csv(r'preprocess/bootstrap/2019_test_%s_%s_20240418.csv'%(city_name, day_type))
        idx_df = pd.read_csv(r'preprocess/bootstrap/bootstrap_index_%s_%s_over_extreme_20240501.csv'%(city_name, day_type))
        n_est, max_d = [50, 4]
        ada_n_est = 50
        ...
        ###
            result_df.to_csv(r'results/bootstrapped/comparison/%s/%s/y_val_over_extreme_%s.csv'%(city_name, day_type, random_state),index=False)
            print('save %s - %s %s rs %s th results csv file'%(city_name, day_type, random_state, ii))
            print('-'*50)
            ii += 1
