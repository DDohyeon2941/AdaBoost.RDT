...
if __name__ == "__main__":
    ...
    total_round = len(day_types) * len(city_names) * len(avg_thresholds) * len(avg_ratios) * rs_num
    ...
    for city_name, day_type in itertools.product(city_names, day_types):
        ...
        for random_state in np.arange(rs_num):
            ...
            result_df.to_csv(r'results/bootstrapped/proposed/%s/%s/y_val_over_extreme_%s.csv'%(city_name, day_type, random_state ),index=False)
